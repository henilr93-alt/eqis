// One-off: send a marketing Promote-digest sample from a recent ECD report
// that actually has ECD_CHEAPER combos. Recipients via CLI arg
// (comma-separated). Defaults to director@etrav.in.
...
(async () => {
  ...
  let promote = 0;
  hotels.forEach(h => (h.combos || []).forEach(c => { if (c.verdict === 'ECD_CHEAPER') promote++; }));
  console.log('Source report:', path.basename(reportPath));
  console.log('Hotels:', hotels.length, ' Promote combos:', promote);

  const rendered = tpl.renderMarketingDigest({ hotels, runId: report.runId, summary: report.summary });
  ...
  emailHistory.append({
    teamId: 'marketing', teamName: 'Marketing Team', reportType: 'ecd_promote_digest',
